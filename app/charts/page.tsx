...
import { ChartDeleteDialog } from '@/components/charts/dialogs';
import { ChartExportDropdownForList } from '@/components/charts/export';
...
const filterConfigs: Record<string, FilterConfig> = {
  ...
  data_source: { type: 'checkbox' },
  chart_type: { type: 'checkbox' },
...
interface ChartsColumnParams {
  sortState: SortState;
  setSortState: (state: SortState) => void;
  filterState: FilterState;
  setFilterState: (state: FilterState | ((prev: FilterState) => FilterState)) => void;
  uniqueDataSources: { value: string; label: string }[];
  uniqueChartTypes: { value: string; label: string }[];
  isDeleting: number | null;
  isDuplicating: number | null;
  hasPermission: (permission: string) => boolean;
  isFavorited: (id: number) => boolean;
  toggleFavorite: (id: number) => void;
  handleDuplicateChart: (chartId: number, chartTitle: string) => void;
  handleDeleteChart: (chartId: number, chartTitle: string) => void;
  enterSelectionMode: () => void;
}

// Column definitions function (extracted outside component)
function getChartsColumns(params: ChartsColumnParams): ColumnDef<Chart>[] {
  const {
    sortState,
    setSortState,
    filterState,
    ...
    },
    {
      ...
      cell: ({ row }) => (
        <div className="flex items-center gap-2">
          <div className="text-base text-gray-700">{`${row.original.schema_name}.${row.original.table_name}`}</div>
        </div>
      ),
...
export default function ChartsPage() {
  ...
  const filterFn = useCallback(
    (chart: Chart, filterState: FilterState): boolean => {
      ...
      if (!matchesTextFilter(chart.title, titleFilter?.text || '')) return false;
      if (titleFilter?.showFavorites && !isFavorited(chart.id)) return false;
      if (!matchesCheckboxFilter(`${chart.schema_name}.${chart.table_name}`, dataSourceFilter))
        return false;
      if (!matchesCheckboxFilter(chart.chart_type, chartTypeFilter)) return false;
      if (!matchesDateFilter(chart.updated_at, dateFilter)) return false;
      return true;
    },
    [isFavorited]
  );
  ...
  const uniqueDataSources = useMemo(
    () =>
      extractUniqueValues(charts, (chart) => {
        const ds = `${chart.schema_name}.${chart.table_name}`;
        return ds !== '.' ? ds : null;
      }),
    [charts]
  );
  ...
  const selectAllCharts = useCallback(
    () => setSelectedCharts(new Set(filteredAndSortedCharts.map((c) => c.id))),
    [filteredAndSortedCharts]
  );
  const deselectAllCharts = useCallback(() => setSelectedCharts(new Set()), []);
  ...
  const handleDuplicateChart = useCallback(
    async (chartId: number, chartTitle: string) => {
      ...
      try {
        const originalChart = charts.find((c: Chart) => c.id === chartId);
        if (!originalChart) {
          toastError.api('Chart not found');
          return;
        }
        const duplicateTitle = generateDuplicateTitle(
          originalChart.title,
          charts.map((c: Chart) => c.title)
        );
  ...
  const handleBulkDelete = useCallback(async () => {
    ...
    try {
      try {
        ...
        await Promise.all(Array.from(selectedCharts).map((id) => deleteChart(id)));
      }
      await mutate();
  ...
  if (isError) return <ErrorState title="Failed to load charts" onRetry={() => mutate()} />;
