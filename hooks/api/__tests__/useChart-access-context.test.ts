/**
 * Access-context params on the table/map chart-data hooks (Task 6c).
 *
 * The backend (Task 6b) now gates POST /api/charts/chart-data-preview/,
 * .../chart-data-preview/total-rows/, and POST /api/charts/map-data-overlay/
 * on chart_id + dashboard_id when a dashboard-tile is asking (see
 * task-06b-report.md, Part C). Dashboard-view table/map tiles must send
 * them; the chart builder and standalone Charts page must not — they call
 * these same hooks without a chartId/dashboardId, which must produce
 * identical requests to before this change (no regression for those paths).
 */
