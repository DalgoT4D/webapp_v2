...
// Extended Dashboard type for list display
type DashboardListItem = Dashboard & { dashboard_title?: string; changed_by_name?: string };
...
interface DashboardColumnParams {
  sortState: SortState;
  setSortState: (state: SortState) => void;
  filterState: FilterState;
  setFilterState: (state: FilterState | ((prev: FilterState) => FilterState)) => void;
  uniqueOwners: { value: string; label: string }[];
  currentUser: {
    email?: string;
    landing_dashboard_id?: number;
    org_default_dashboard_id?: number;
  } | null;
  ...
  handleDuplicateDashboard: (id: number, title: string) => void;
  handleDeleteDashboard: (id: number, title: string) => void;
  handleShareDashboard: (dashboard: DashboardListItem) => void;
  handleSetPersonalLanding: (id: number) => void;
  handleRemovePersonalLanding: () => void;
  handleSetOrgDefault: (id: number) => void;
}

// Column definitions function (extracted outside component)
function getDashboardColumns(params: DashboardColumnParams): ColumnDef<DashboardListItem>[] {
  const {
    sortState,
    setSortState,
    filterState,
...
export function DashboardListV2() {
  ...
  const filterFn = useCallback(
    (dashboard: DashboardListItem, filterState: FilterState): boolean => {
      ...
      if (!matchesTextFilter(title, nameFilter?.text || '')) return false;
      if (nameFilter?.showFavorites && !isFavorited(dashboard.id)) return false;
      if (nameFilter?.showLocked && !dashboard.is_locked) return false;
      if (nameFilter?.showShared && !dashboard.is_public) return false;
      const owner = dashboard.created_by || dashboard.changed_by_name || 'Unknown';
      if (!matchesCheckboxFilter(owner, ownerFilter)) return false;
      if (!matchesDateFilter(dashboard.updated_at, dateFilter)) return false;
      return true;
    },
    [isFavorited]
  );
  ...
  const uniqueOwners = useMemo(
    () =>
      extractUniqueValues(dashboards, (d) => {
        const owner = d.created_by || d.changed_by_name;
  ...
  const pinnedDashboards = useMemo(
    () =>
      filteredAndSortedDashboards.filter((d) => {
        const isPersonalLanding = currentUser?.landing_dashboard_id === d.id;
        const isOrgDefault = currentUser?.org_default_dashboard_id === d.id;
  ...
  if (isError) return <ErrorState title="Failed to load dashboards" onRetry={() => mutate()} />;
