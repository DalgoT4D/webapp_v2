...
import { useAdminOrgs, useAdminNotificationActions } from '@/hooks/api/useAdminPortal';
...
interface BroadcastComposerProps {
  /** Refresh the history tab after a successful send. */
  onSent: () => void;
}

/**
 * The broadcast composer — the "Create notification" tab. Audience is whole
 * platform, one org, or several orgs at once, resolved server-side into ONE merged
 * recipient list, never a per-org breakdown. Channels (in-app / email) are
 * admin-chosen per broadcast; at least one must be on, enforced here AND
 * server-side. Immediate send only — no scheduling, no cancel (plan.md Milestone 2).
 */
