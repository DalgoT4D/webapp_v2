...
describe('Admin ChangeRoleDialog', () => {
  it('preselects the current role and submits it with the org + user ids', async () => {
    ...
    await waitFor(() =>
      expect(mockTrackEvent).toHaveBeenCalledWith(ANALYTICS_EVENTS.ADMIN_USER_ROLE_CHANGED, {
        role: 'guest',
      })
    );
  });

  it('does not report a role change that failed', async () => {
    mockChangeRole.mockRejectedValueOnce(new Error('backend down'));
    const user = userEvent.setup();
    render(
      <ChangeRoleDialog
        open
    ...
    await waitFor(() => expect(mockChangeRole).toHaveBeenCalled());
