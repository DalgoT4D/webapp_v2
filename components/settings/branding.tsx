...
const PRESET_PALETTES = PRESET_CHART_PALETTES.map((p) => ({
  ...
  colors: getSolidColors(p.colors),
}));

const DEFAULT_PALETTE = DEFAULT_CHART_PALETTE_COLORS;
...
function PaletteEditor({ paletteName, paletteColors, onColorChange }: PaletteEditorProps) {
  ...
  return (
    <div className="space-y-2">
      <Label>
        Active Palette
        {paletteName && (
          <span className="ml-2 text-xs text-muted-foreground font-normal">({paletteName})</span>
        )}
      </Label>
      <div className="flex gap-2 flex-wrap">
        {paletteColors.map((color, index) => (
          <div key={color + index} className="relative">
            ...
            />
            <span className="text-[9px] text-gray-400 block text-center mt-0.5">{index + 1}</span>
          </div>
        ))}
      </div>
      ...
      {editingColorIndex !== null && (
        ...
        >
          ...
          <HexColorPicker
            ...
            onChange={(newColor) => onColorChange(editingColorIndex, newColor)}
          ...
          <Input
            ...
            onChange={(e) => {
              ...
              if (/^#[0-9a-fA-F]{0,6}$/.test(val)) {
                onColorChange(editingColorIndex, val);
              }
            }}
        ...
        <div className="border rounded-lg p-4 bg-white flex items-end gap-1.5 h-24">
          {paletteColors.map((color, i) => (
            <div
              ...
              style={{ backgroundColor: color, height: `${30 + ((i * 17 + 23) % 70)}%` }}
            />
          ))}
        </div>
      </div>
