...
export function getConnectionHelp(opts: HelpOptions = {}): ConnectionConcept[] {
  ...
  cards.push({
    ...
    body: supportsColumnCasting
      ? 'Use the chevron at the right of a sheet row to expand its columns. Each Column type starts with the detected source type; change it when needed, then confirm every selected column before continuing. You can use these column settings without turning on Advanced per-table settings.'
