...
- **Which auth route a Google Sheets source used can't be read back** — the managed option leaves credentials empty on purpose, so empty means "Dalgo fills it in", not "nothing chosen". The form reports it up via `onAuthModeChange` (alongside `onAuthSatisfiedChange`) and `SOURCE_CREATED` carries `auth_mode` from `SOURCE_AUTH_MODES` (`oauth` / `managed_key` / `own_key`). Create-time only: Airbyte returns a stored key masked, so this cannot be reported on edit.
- **Column casting is reported as one boolean, never itemised** — `CONNECTION_CREATED` / `CONNECTION_UPDATED` carry `has_post_sync_transform`, taken from the payload that was actually sent (`buildPostSyncTransform()` is called once and reused, so the event can't drift from the request). The transform's contents are schema/table/column names and cast types — warehouse data that must not reach PostHog (`connection-cast-analytics.test.ts` guards this).
