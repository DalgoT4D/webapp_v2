...
- **Column casting is analytics-summarised, never itemised** — `CONNECTION_CREATED` / `CONNECTION_UPDATED` carry `casting_used` + `cast_column_count`. Column names and cast types are warehouse data and must not reach PostHog (`connection-cast-analytics.test.ts` guards this).
